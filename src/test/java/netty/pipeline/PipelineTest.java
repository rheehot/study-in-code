...
public class PipelineTest {
    ...
    void status_update_and_log_pipeline() {
        ...
        channel.pipeline()
               .addLast(new StatusUpdater(new DeviceStatus()))
               .addLast(new Logger());

        // When : 부분 상태 메시지를 채널 입력으로 쓰기
        channel.writeInbound(new RunningStatus(10, 100.0));

        // Then : 파이프라인을 통과한 객체는 입력 타입과 동일
        Object throughMessage = channel.readInbound();
        Assertions.assertThat(throughMessage.getClass()).isEqualTo(RunningStatus.class);
    }
}
